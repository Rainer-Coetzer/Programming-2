...
class WeatherGUI extends WeatherFrame {
    ...
    private void createSettingsTab() {
        ...
        JLabel infoLabel = new JLabel("Weather App v1.0 - © 2023");
        ...
        refreshButton.addActionListener(e -> {
            try {
                if (db != null) {
                    ...
                    sb.append(String.format("%-20s %-25s %-10s %-10s %-30s\n", 
                        "City", "Date", "Temp", "Wind", "Forecast"));
                    sb.append("----------------------------------------------------------------------------------------\n");
                    
                    for (WeatherHistory item : history) {
                        sb.append(String.format("%-20s %-25s %-10.1f %-10.1f %-30s\n", 
                            item.getCity(), 
                            item.getSearchDate(), 
                            item.getTemperature(),
                            item.getWindSpeed(),
                            item.getForecastData().length() > 30 ? 
                                item.getForecastData().substring(0, 27) + "..." : 
                                item.getForecastData()));
                    }
    ...
    private void fetchWeather() {
        ...
        try {
            ...
            if (db != null) {
                try {
                    db.saveWeatherSearch(city, data);
                } catch (SQLException e) {
...
class WeatherDatabase {
    ...
    public void saveWeatherSearch(String city, WeatherData data) throws SQLException {
        String sql = "INSERT INTO weather_history(city, temperature, wind_speed, forecast_data) VALUES(?, ?, ?, ?)";
        
        // Convert forecast data to string for storage
        StringBuilder forecastStr = new StringBuilder();
        for (DailyForecast day : data.getForecast()) {
            forecastStr.append(String.format("%s: %.1f/%.1f; ", 
                day.getDate(), day.getMaxTemp(), day.getMinTemp()));
        }
        
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, city);
            pstmt.setDouble(2, data.getCurrentTemp());
            pstmt.setDouble(3, data.getWindSpeed());
            pstmt.setString(4, forecastStr.toString());
            pstmt.executeUpdate();
        }
    }
    ...
    public List<WeatherHistory> getWeatherHistory() throws SQLException {
        ...
        String sql = "SELECT city, temperature, wind_speed, forecast_data, search_date " +
                     "FROM weather_history ORDER BY search_date DESC LIMIT 50";
...
class WeatherHistory {
    ...
    public WeatherHistory(String city, double temperature, double windSpeed, 
                         String forecastData, String searchDate) {
        this.city = city;
